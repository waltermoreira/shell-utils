...
lib.makeOverridable (
  ...
  let
    promptName = "nix" + lib.optionalString (name != "") " " + name;
    histFile = "~/.history-" + (if name == "" then "noname" else name);
    zshConfig = writeTextFile {
      name = "zshrc";
      text = ''
    ...
    myStarshipConfig = with lib; fix (
      extends
        (self: super: { format = "[\\[${promptName}\\]](green)" + super.format; })
        (self: baseStarshipConfig // starshipConfig)
    );
    config = (formats.toml { }).generate "starship.toml" myStarshipConfig;
